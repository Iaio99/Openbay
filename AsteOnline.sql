...
CREATE TABLE `Oggetti` (
  ...
  `Dimensioni` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL,
  `Descrizione` blob NOT NULL,
  `BaseAsta` float NOT NULL,
...
/*!50001 VIEW `Aste` AS select `Oggetti`.`Codice` AS `Codice`,`Oggetti`.`Stato` AS `Stato`,`Oggetti`.`Dimensioni` AS `Dimensioni`,`Oggetti`.`Descrizione` AS `Descrizione`,`Oggetti`.`BaseAsta` AS `BaseAsta`,`Oggetti`.`ScadenzaAsta` AS `ScadenzaAsta`,`Oggetti`.`NumeroOfferte` AS `NumeroOfferte`,`Oggetti`.`PrimoLivello` AS `PrimoLivello`,`Oggetti`.`SecondoLivello` AS `SecondoLivello`,`Oggetti`.`TerzoLivello` AS `TerzoLivello`,max(`Offerte`.`Importo`) AS `OffertaMassima` from (`Offerte` join `Oggetti` on((`Offerte`.`Oggetto` = `Oggetti`.`Codice`))) */;
...
-- Dump completed on 2023-02-09 13:17:01
