...
bool init_validation(void)
{
	...
	ret1 = regcomp(&regex_cf, "^(([A-Z]|[a-z]){6}[0-9]{2}([A-Z]|[a-z])[0-9]{2}([A-Z]|[a-z])[0-9]{3}([A-Z]|[a-z]))", REG_EXTENDED);
	if(ret1) {
		if(ret1 == REG_ESPACE) {
			fprintf(stderr, "%s\n", strerror(ENOMEM));
		} else {
	...
	ret2 = regcomp(&regex_credit_card_number, "^([0-9]{4}-[0-9]{4}-[0-9]{4}-[0-9]{4})", REG_EXTENDED);
	if (ret2) {
		if(ret2 == REG_ESPACE) {
			fprintf(stderr, "%s\n", strerror(ENOMEM));
		} else {
	...
	ret3 = regcomp(&regex_cvv, "^([0-9]{3})", REG_EXTENDED);
	if (ret3) {
		if(ret3 == REG_ESPACE) {
			fprintf(stderr, "%s\n", strerror(ENOMEM));
		} else {
	...
	ret4 = regcomp(&regex_date, "^([12][0-9]{3}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01]))", REG_EXTENDED);
	if(ret4) {
		if(ret4 == REG_ESPACE) {
			fprintf(stderr, "%s\n", strerror(ENOMEM));
		} else {
	...
	ret5 = regcomp(&regex_time, "^([0-1]?[0-9]|2?[0-3]|[0-9]):([0-5][0-9]|[0-9])$", REG_EXTENDED);
	if(ret5) {
		if(ret5 == REG_ESPACE) {
			fprintf(stderr, "%s\n", strerror(ENOMEM));
		} else {
